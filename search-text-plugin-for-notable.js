...
/*****************************************************
* Search Text Plugin for Notable v1.8.4
* Version: 0.1.1
* Author: DevTomek
* Website: https://devtomek.pl
*****************************************************/
var markInstance = new Mark(document.querySelector('.layout-content.preview'));
var currentSelectedMarkIndex, selectedMarkCount = 0;

document.querySelector('head').insertAdjacentHTML('afterBegin',`
    <style>
        .selected {
            background-color: rgba(255, 35, 0, 0.57) !important;
        }
    </style>
`);

document.querySelector('.mainbar.layout.column').insertAdjacentHTML('afterBegin',`
    <div class="layout-header toolbar search-text-js">
        <div class="multiple grow">
            <div class="multiple joined no-separators grow search">
                <input type="search" name="searchText" class="bordered grow small" placeholder="Search text..." value="" onkeypress="searchNext()">
...
var markInstance = previewEl ? new Mark(previewEl) : null;
var currentSelectedMarkIndex = 0, selectedMarkCount = 0;

// تحديث العدّاد
window.updateSearchCountLabel = function () {
...
window.search = function (searchPhrase) {
    ...
    var options = {separateWordSearch: false, diacritics: true, debug: false};
    markInstance.unmark({
        done: function() {
            markInstance.mark(searchPhrase, options);
        }
    });
    currentSelectedMarkIndex = 0;
    selectedMarkCount = document.querySelectorAll('.layout-content.preview mark').length;
    updateSearchCountLabel();
    searchNext();
    document.querySelector('button[data-search="clear"]').hidden = searchPhrase == '';
}

// استماع على الكتابة
document.querySelector('input[name="searchText"]').addEventListener('input', function(event) {
...
window.searchNext = function () {
    ...
    markElements.forEach(function(element, index) {
        if (currentSelectedMarkIndex === index) {
            ...
            element.scrollIntoView({ behavior: "smooth", block: "center" }); // ✅ إصلاح
        } else {
...
document.querySelector('button[data-search="clear"]').addEventListener('click', resetSearch);
